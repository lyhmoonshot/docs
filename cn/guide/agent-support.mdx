...
- **预算控制**
  - **设置日消费上限**：在使用前，请前往[Kimi 开放平台项目设置](/console/projects/settings)配置「项目日消费预算」。一旦达到预算上限，系统将自动拒绝该项目下所有 API 请求（注：由于计费延迟，限制生效可能有约 10 分钟延迟）。设置方式请见[组织管理最佳实践](/guide/org-best-practice)
...
* 访问[开放平台](/console/api-keys)创建获取 API Key，选择 default 默认项目。
