...
如果你想避免出现 `finish_reason=length`，我们建议你放大 `max_tokens` 的值，我们推荐的最佳实践是：通过 [estimate-token-count](/docs/api/estimate) 接口计算输入内容的 Tokens 数量，随后使用 Kimi 大模型所支持的最大 Tokens 数量（例如，对于 `moonshot-v1-32k` 模型，它最大支持 32k Tokens），则可以设置的最大请求的 `max_tokens` 值是 32k。
