...
上一节的例子中，我们的 `image_url` 为base64编码后的图片。由于视频文件往往更大，所以我们还提供额外方法，您可以先上传图片或视频到 Moonshot，然后通过文件 ID 引用。关于图片或视频的上传，请参阅 [图片理解上传](/docs/api/files-upload)
...
关于token价格，详见 [模型推理价格说明](/docs/pricing/chat-k25)
...
在 [chat](/docs/api/chat) 文档中有一系列参数，但对于k2.5系列模型，其行为会有所不同
