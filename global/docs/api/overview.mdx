...
When using SDKs, set `base_url` / `baseURL` to `https://api.moonshot.ai/v1`. When calling HTTP endpoints directly, use the full path such as `https://api.moonshot.ai/v1/chat/completions`.
...
For the full list of error types, messages, and troubleshooting tips, see [Errors](/docs/api/errors).

## API Endpoints

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/v1/chat/completions` | POST | [Create Chat Completion](/docs/api/chat) |
| `/v1/models` | GET | [List Models](/docs/api/list-models) |
| `/v1/tokenizers/estimate-token-count` | POST | [Estimate Tokens](/docs/api/estimate) |
| `/v1/users/me/balance` | GET | [Check Balance](/docs/api/balance) |
| `/v1/files` | POST | [Upload File](/docs/api/files-upload) |
| `/v1/files` | GET | [List Files](/docs/api/files-list) |
| `/v1/files/{file_id}` | GET | [Get File Info](/docs/api/files-retrieve) |
| `/v1/files/{file_id}` | DELETE | [Delete File](/docs/api/files-delete) |
| `/v1/files/{file_id}/content` | GET | [Get File Content](/docs/api/files-content) |
