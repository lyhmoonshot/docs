...
You can use our Chat Completions API to send requests. You need to provide an API key and a model name. You can choose to use the default max\_tokens parameter or customize the max\_tokens parameter. You can refer to the [Chat API documentation](/docs/api/chat) for the calling method.
