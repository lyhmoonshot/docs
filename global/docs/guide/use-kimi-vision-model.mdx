...
In the previous example, our `image_url` was a base64-encoded image. Since video files are often larger, we provide an additional method where you can first upload images or videos to Moonshot, then reference them via file ID. For information on uploading images or videos, please refer to [Image Understanding Upload](/api/files)
...
Regarding file upload limitations, please refer to the [File Upload](/api/files) documentation.
