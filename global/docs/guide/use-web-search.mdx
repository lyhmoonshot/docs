...
In the previous chapter ([Using Kimi API to Complete Tool Calls](/docs/guide/use-kimi-api-to-complete-tool-calls)), we explained in detail how to use the `tool_calls` feature of the Kimi API to enable the Kimi large language model to perform internet searches. Let's review the process we implemented:
...
In addition to token consumption, we also charge a call fee for each web search, priced at \$0.005. For more details, please refer to [Pricing](/docs/pricing/tools).
