---
title: "API Integration with OpenClaw: Connect Kimi K2.5 Step by Step"
sidebarTitle: "Use Kimi K2.5 in OpenClaw"
description: Build a cross-platform AI agent with OpenClaw and the Kimi K2.5 API. Follow our guide to install OpenClaw and configure your Moonshot API key for seamless chat on Telegram, Discord, Slack, and WhatsApp.
canonical: https://platform.moonshot.ai/docs/guide/use-kimi-in-openclaw
area: "guide"
models:
  - "kimi-k2.5"
apis:
  - "/v1/chat/completions"
capabilities:
  - "agent"
  - "multi-turn-chat"
  - "tool-use"
  - "integration"
level: "intermediate"
audience:
  - "developer"
  - "ml-engineer"
related:
  - "docs/guide/kimi-k2-quickstart"
  - "docs/guide/use-kimi-k2-thinking-model"
  - "docs/pricing/chat"
aliases:
  - "openclaw"
  - "openclaw integration"
  - "kimi k2.5 openclaw"
---

# How to Connect Kimi K2.5 to OpenClaw

![openclaw](/public/docs/guide/openclaw/clawbot.jpeg)

OpenClaw (formerly Clawdbot and Moltbot) is an open-source, self-hosted AI agent platform that lets you run AI assistants locally. It integrates with messaging apps like WhatsApp, Telegram, Discord, Slack, and Signal to connect large language models to real-world workflows. The platform supports multiple LLM providers, extensible skills, and gives you full control over your data and API keys.

Here is how to use the Kimi K2.5 model in OpenClaw.

## Step1: Create Kimi Platform API Key

* Newly registered accounts have no balance and need to be topped up before use. It is recommended to recharge $20 or more to upgrade to tier2 to get a smoother usage experience [Go to recharge](https://platform.moonshot.ai/console/pay)
* Create and copy the API key: [Go to create](https://platform.moonshot.ai/console/api-keys)

![recharge](/public/docs/guide/openclaw/recharge.png)


## Step2: Install OpenClaw

Run the following command in the terminal. If you have already installed OpenClaw, please also execute this command to upgrade OpenClaw.

**Version: We strongly recommend upgrading to version 2026.2.3 or above. Starting from 2.3, OpenClaw supports selecting the Kimi K2.5 models for both Mainland China and International Open Platform.**

```curl
curl -fsSL https://openclaw.ai/install.sh | bash
```


![download](/public/docs/guide/openclaw/download.png)

Visit the official OpenClaw website for more information: https://openclaw.ai/




If the installation is successful, you will see the following prompt, indicating successful installation:

![success](/public/docs/guide/openclaw/success.png)

Select [yes] and continue with the installation:

![yes](/public/docs/guide/openclaw/yes.png)

Select the QuickStart option:

![quickstart](/public/docs/guide/openclaw/quickstart.png)

## Step3: Set up Kimi K2.5

* **Step 1: Model.auth provider > Choose Moonshot AI (Kimi K2.5)**
* **Step 2: Model AI auth method(Kimi K2.5) > Choose Kimi API key (.ai)**
* **Step 3: Enter Moonshot API Key(.ai) > Enter your API Key**
* **Step 4: Default model > Keep current（moonshot/kimi-k2.5）**


![1](/public/docs/guide/openclaw/1.png)

![3](/public/docs/guide/openclaw/3.png)

![models](/public/docs/guide/openclaw/models.png)

![default_models](/public/docs/guide/openclaw/default_models.png)


Then you will see the option to select the chat tool. You can select [Skip for now] first:

![4](/public/docs/guide/openclaw/4.png)

Other configurations, such as the Gateway Port setting, can be left at the default 18789. For Skills and the package manager, select npm or other options, and you can select Yes for all the remaining options.

![5](/public/docs/guide/openclaw/5.png)

For skills, you can skip this step or use the space key to select the ones you need.

![6](/public/docs/guide/openclaw/6.png)

For the remaining API keys, select no if you don't have them:

![7](/public/docs/guide/openclaw/7.png)

The last three hooks can be enabled to perform content guidance logs and session records:

![8](/public/docs/guide/openclaw/8.png)

After installation, you can automatically access http://127.0.0.1:18789 to open the chat interface and start working.

![chat](/public/docs/guide/openclaw/chat.jpeg)



