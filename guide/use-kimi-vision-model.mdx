...
sidebarTitle: "Kimi Vision Model"
description: "How to use Kimi's multi-modal vision models (images and video) via the Chat API, including base64 uploads, file IDs, supported formats, and token cost behavior."
canonical: https://platform.moonshot.ai/docs/guide/use-kimi-vision-model
area: "guide"
models:
  - "kimi-k2.5"
...
In the previous example, our `image_url` was a base64-encoded image. Since video files are often larger, we provide an additional method where you can first upload images or videos to Moonshot, then reference them via file ID. For information on uploading images or videos, please refer to [Tool Use API](/docs/api/tool-use)
...
[Model Inference Pricing](/docs/pricing/chat)
...
Regarding file upload limitations, please refer to the [Tool Use API](/docs/api/tool-use) documentation.
...
Parameters are listed in [Tool Use API](/docs/api/tool-use). However, behaviour of some parameters may be different in k2.5 models.
...
| thinking | optional | **New!** This parameter controls if the thinking is enabled for this request. Only available for `kimi-k2.5` model. | object | Default to be `{"type": "enabled"}`. Value can only be one of `{"type": "enabled"}` or `{"type": "disabled"}` |
...
### Configuring the `thinking` Parameter in OpenAI SDK
...
**To enable thinking (default behavior):**
